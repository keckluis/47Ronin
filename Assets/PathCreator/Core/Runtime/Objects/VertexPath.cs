        ...
        TimeOnPathData CalculatePercentOnPathData (float t, EndOfPathInstruction endOfPathInstruction) {
            ...
            switch (endOfPathInstruction) {
                ...
                case EndOfPathInstruction.Reverse:
                    ...
                    if (t > 1f - 0.001f && !reverse)
                    {
                        reverse = true;
                        follower.transform.localScale = new Vector3(follower.transform.localScale.x * -1, follower.transform.localScale.y, follower.transform.localScale.z);
                        Debug.Log("r");
                    }
                    else if (t < 0f + 0.001f && reverse)
                    {
                        reverse = false;
                        follower.transform.localScale = new Vector3(follower.transform.localScale.x * -1, follower.transform.localScale.y, follower.transform.localScale.z);
                        Debug.Log("l");
